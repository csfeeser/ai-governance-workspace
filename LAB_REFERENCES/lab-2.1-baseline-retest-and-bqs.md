...
In this lab you worked out which export can actually show answer quality, marked a sample against a written standard, read the Business Quality Score it produced, measured it against the baseline and an alarm level set in advance, traced the drop to its most likely cause, and decided who needed to hear about it. This is the periodic control test that produces evidence rather than reassurance. You have run it once here, with the standard and most of the sample supplied; what transfers to a workflow of your own is the method, not this single run. Lab 2.2 shows how to prove the cause by ruling out the other possibilities, and how to write it up as a test record.
